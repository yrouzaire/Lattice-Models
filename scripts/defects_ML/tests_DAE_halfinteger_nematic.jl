...
ind = rand(1:63)
    ...
    p2=plot_thetas(recon,model,lattice,defects=false,title="µ = $(round(infer_mu(mod.(recon,2pi),q=CHARGE),digits=2))")
...
z = @elapsed for i in each(mus)
    ...
    for j in each(flip_strength) , r in 1:R
        ...
        recon = NN_test(X_noisy_reshaped)[:,:,1,1]
        mus_infered[i,j,r] = infer_mu(recon,q=CHARGE)
    end
end
...
update!(thetasagg,model,latticeagg,tmax=400)  # updates until time = t
