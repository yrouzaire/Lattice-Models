...
# Dynamics of a µ0, for several \sigma
reals   = 20
    tmax    = 50 ; every = 1 ; times = collect(0:every:tmax+every)
    µ0s     = [0,pi/2,pi,3pi/2]
    µ0s     = [pi/2 - 0.5,pi/2 + 0.5]
    visions = [0,0.05,0.1,0.2]
# Basin of attraction
reals    = 20
    tmax = 50 ; every = 1 ; times = collect(0:every:tmax+every)
    dµ   = pi/4 ; µ0s = collect(1:dµ:5.25-dµ)
    visions = 0.2

mus = NaN*zeros(length(µ0s),length(visions),length(times)-1,reals)
    z = @elapsed for i in each(µ0s)
    for j in each(visions)
        println("µ0 = ",round(µ0s[i],digits=2)," ; vision = ",visions[j])
...
prinz(z) # 5 minutes for a lot of curves, very fast if L = 15
...
mus_avg = nanmean(mus,4)[:,:,:,1]
    ...
    for j in 1:length(each(visions))
    ...
    p
# savefig("NRI/one_defect/µt.png")
p=plot(ylims=(0,2pi),xlabel="σt",ylabel="µ")
    for i in 1:length(µ0s)-1
    ...
    # plot!(0.1times[5:135],pi/2*exp.(-1.5(0.1times[5:135].-0.2)).+pi/2,c=:black,lw=2)
    plot!(t->pi/2-2atan(tanh(-(t/2))),c=:black,lw=2,line=:dash)
    p
# savefig("NRI/one_defect/µt_collapse.png")
mus_avg = nanmean(mus,4)[:,:,:,1]
...
tspan = (0.0,50.0)
    ...
    plot!(t->pi,c=:black,lw=2,line=:dash)#,label="σ = $(visions[j])",rib=0)
    # plot!(t->pi/2,c=:black,lw=2,line=:dashdot)#,label="σ = $(visions[j])",rib=0)
    p
# savefig("NRI/one_defect/traj_analytic_decayµ_q+.png")
p=plot(legend=false,ylims=(0,2pi),ylabel="µ(t)",size=(400,400))
    tspan = (0.0,50.0)
    token_col = 1
    for sig in [0,0.05,0.1,0.2]
        f(u,p,t) = sig*sin(u)
        prob = ODEProblem(f,Float64(pi/2),tspan)
        sol = solve(prob, Tsit5(), reltol=1e-8, abstol=1e-8)
        plot!(sol,lw=1.5,c=token_col,label="σ = $sig",rib=0)

        prob = ODEProblem(f,Float64(3pi/2),tspan)
        sol = solve(prob, Tsit5(), reltol=1e-8, abstol=1e-8)
        plot!(sol,lw=1.5,c=token_col,label="σ = $sig",rib=0)
        token_col += 1
    end
    ...
    p
# savefig("NRI/one_defect/traj_analytic_decayµ_sink_q+.png")
...
include(srcdir("../parameters.jl"));
    ...
    prinz(z)
    mp4(anim,datadir("../NRI/one_defect/NRI_µ$(params["mu0"])_q-1.mp4"))
